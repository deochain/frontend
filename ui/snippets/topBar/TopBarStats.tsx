...
const TopBarStats = () => {
  ...
  const FALLBACK_GAS_PRICE = 30;

  const { data, isPlaceholderData, isError, refetch, dataUpdatedAt } =
    useApiQuery("stats", {
      queryOptions: {
  ...
  return (
    <Flex alignItems="center" fontSize="xs" fontWeight={500}>
        ...
        <Flex columnGap={1}>
          ...
          {data.coin_price_change_percentage && (
            <Skeleton isLoaded={!isPlaceholderData}>
              <chakra.span
                color={
                  Number(data.coin_price_change_percentage) >= 0
                    ? "green.500"
                    : "red.500"
                }
              >
      ...
      config.features.gasTracker.isEnabled ? (
        ...
        <Flex columnGap={1}>
          <chakra.span color="text_secondary">Gas Price: </chakra.span>
          <span>0.01</span>
        </Flex>
      )}
      <Flex columnGap={1} px={3}>
      ...
      <Flex columnGap={1}>
        ...
        <span>
          $
          {Number("0.12").toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 6,
          })}
        </span>
