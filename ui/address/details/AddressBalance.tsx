...
const AddressBalance = ({ data, isLoading }: Props) => {
  const [lastBlockNumber, setLastBlockNumber] = React.useState<number>(
  ...
  const updateData = React.useCallback(
    (balance: string, exchangeRate: string, blockNumber: number) => {
      ...
      const queryKey = getResourceKey("address", {
  ...
  const handleNewBalanceMessage: SocketMessage.AddressBalance["handler"] =
    React.useCallback(
      (payload) => {
        updateData(
          payload.balance,
  ...
  const handleNewCoinBalanceMessage: SocketMessage.AddressCurrentCoinBalance["handler"] =
    React.useCallback(
      (payload) => {
        updateData(
