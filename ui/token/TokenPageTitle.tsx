...
const TokenPageTitle = ({ tokenQuery, addressQuery, hash }: Props) => {
  ...
  const iconUrl = `https://raw.githubusercontent.com/rajneeshNadcab/inertia-images/main/icons/${hash}/icon.png`;
  ...
  const tags: Array<EntityTag> = React.useMemo(() => {
    return [
      ...
      ...(addressMetadataQuery.data?.addresses?.[
        hash.toLowerCase()
      ]?.tags.filter((tag) => tag.tagType !== "note") || []),
    ]
      .filter(Boolean)
      .sort(sortEntityTags);
  }, [
  ...
  const secondRow = (
    ...
    >
      ...
      >
        ...
        <NetworkExplorers
          type="token"
          pathParam={addressHash.toLowerCase()}
          ml={{ base: "auto", lg: 0 }}
        />
      </Flex>
    </Flex>
